...
import { Star, Globe, Clock, Award, Smartphone, Infinity as InfinityIcon, Play, ChevronDown, Check, User, Globe as GlobeIcon, Heart, ShoppingBag, Loader2, X } from 'lucide-react';
import { courseService, moduleService, lessonService } from '../services/courseService';
...
export const CourseDetail: React.FC<CourseDetailProps> = ({
  ...
  const fetchCourseDetail = async (courseSlug: string) => {
    try {
      ...
      const courseData: CourseData = courseResponse;
      setCourse(courseData);

      // Fetch modules for this course
      const modulesResponse = await moduleService.getModulesByCourse(courseData._id);
      ...
      const modulesWithLessons = await Promise.all(
        modules.map(async (module) => {
          try {
            ...
            const lessons: Lesson[] = (lessonsResponse || []).sort((a: Lesson, b: Lesson) => a.order - b.order);
            return { ...module, lessons };
          } catch (error) {
            console.error(`Error fetching lessons for module ${module._id}:`, error);
            return { ...module, lessons: [] };
          }
        })
      );
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-sm text-brand-gray mt-4 font-semibold">Loading course details...</p>
      </div>
    );
  }
  ...
  const isAddedToCart = cart.includes(course._id);
  ...
  const instructorName = course.primaryInstructor 
    ? (typeof course.primaryInstructor === 'object' ? course.primaryInstructor.name : 'Instructor')
    : 'Course Instructor';
  const instructorRole = 'Expert Instructor';
  const instructorBio = 'Learn from the best in the industry.';
  const instructorAvatar = 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150';
        ...
        <div className="max-w-4xl relative z-10 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-xs font-bold text-white/70">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-white font-extrabold">{course.rating.toFixed(1)}</span>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 lg:hidden">
            ...
            <button
              onClick={() => onToggleCart(course._id)}
            ...
            {chapters.length > 0 ? (
              <div className="border border-brand-border rounded-3xl bg-white overflow-hidden premium-shadow divide-y divide-brand-border/60">
                {chapters.map((chapter) => {
                  ...
                  const chapterDuration = chapter.lessons.reduce((acc, l) => acc + l.duration, 0);
                      ...
                      >
                        <div className="space-y-1 pr-4">
                          ...
                          <p className="text-[10px] text-brand-gray font-bold uppercase tracking-wider">
                            {chapter.lessons.length} lectures • {formatDuration(chapterDuration)}
                          </p>
                        </div>
                          ...
                          >
                            <div className="px-6 pb-4 pt-1 divide-y divide-brand-border/40">
                              {chapter.lessons.map((lesson) => (
                                <div key={lesson._id} className="py-3 flex items-center justify-between gap-4 text-xs">
                                  ...
                                  <div className="flex items-center gap-3 shrink-0">
                                    <span className="text-brand-gray font-bold">{formatLessonDuration(lesson.duration)}</span>
                                    {lesson.isPreview && (
                                      <button 
              ...
              <div className="border border-brand-border rounded-3xl bg-white p-8 text-center">
                <p className="text-sm text-brand-gray font-semibold">No curriculum available yet</p>
              </div>
            )}
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="bg-white border border-brand-border rounded-[32px] premium-shadow p-6 flex flex-col sm:flex-row gap-6">
              ...
              <div className="space-y-3 flex-1">
                <div>
                  <h4 className="font-extrabold text-lg text-brand-navy">{instructorName}</h4>
                  <p className="text-xs font-bold text-brand-purple mt-0.5">{instructorRole}</p>
                </div>
          ...
          <div className="bg-white rounded-[32px] border border-brand-border premium-shadow p-6 space-y-6">
            ...
            <div className="space-y-1">
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-brand-navy">${displayPrice}</span>
                {course.discountPrice && (
                  <span className="text-sm text-brand-gray line-through font-bold">${course.price}</span>
                )}
              </div>
              {course.discountPrice && (
                <p className="text-[10px] text-red-500 font-extrabold">🔥 Limited offer: {Math.round(((course.price - course.discountPrice) / course.price) * 100)}% discount included!</p>
              )}
            </div>
