...
import { ChevronLeft, ChevronRight, BookOpen, FileText, Link2 } from 'lucide-react';
...
import { fetchCourseProgress, updateVideoProgress } from '../../store/progressSlice';
...
export function CoursePlayerPage() {
  const { slug, lessonId: urlLessonId } = useParams<{ slug: string; lessonId?: string }>();
  const navigate = useNavigate();
  const courseSlug = slug || '';
  const dispatch = useAppDispatch();
  
  const reduxLessonProgress = useAppSelector(state => state.progress.lessonProgress);
  const reduxCourseProgresses = useAppSelector(state => state.progress.courseProgresses);
  const reduxCompletedLessonsList = useAppSelector(state => state.progress.completedLessonsList);
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const [modulesResponse, enrollments, progressResult] = await Promise.all([
          moduleService.getModulesByCourse(courseData._id).catch(() => []),
          enrollmentService.getMyEnrollments().catch(() => []),
          dispatch(fetchCourseProgress(courseData._id)).unwrap().catch(() => null)
        ]);
            ...
            try {
              ...
              const transformedLessons: Lesson[] = lessonsResponse && Array.isArray(lessonsResponse)
                ? lessonsResponse.map((apiLesson) => ({
                    ...
                    duration: Math.round((apiLesson.durationSeconds || 0) / 60),
        ...
        if (courseEnrollment) {
          ...
          const lastWatchedLessonId = getLessonId(progressResult?.data?.lastWatched);
          const currentEnrollmentLessonId = getLessonId(courseEnrollment?.currentLesson);
  ...
  const completedLessons = useMemo(() => {
    ...
    return Array.from(new Set([...courseCompletedLessons, ...completedFromLessonProgress]))
      .filter(lessonId => courseLessonIds.has(lessonId));
  }, [course?._id, courseLessonIds, reduxCompletedLessonsList, reduxLessonProgress]);
  const completedCount = completedLessons.length;
  const progressPercentage = course?._id ? (reduxCourseProgresses[course._id] ?? 0) : 0;
  ...
  return (
    <div className="w-full bg-bg-primary text-brand-navy min-h-screen">
          ...
          {lesson && (
            <>
              ...
              <VideoPlayer
                ...
                initialTime={reduxLessonProgress[lesson._id]?.watchedSeconds || 0}
              ...
              <div className="flex items-center justify-between mt-4">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-brand-gray hover:text-brand-purple disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
              ...
              {/* Lesson Information */}
              <div className="mt-6">
                <h1 className="text-2xl font-semibold text-brand-navy mb-2">{lesson.title}</h1>
                <p className="text-brand-gray mb-6">{lesson.description || 'No description available'}</p>
                    ...
                    {(['overview', 'notes', 'resources'] as const).map((tab) => (
                      <button
                        ...
                        className={`pb-4 text-sm font-medium transition-colors relative ${
                          activeTab === tab ? 'text-brand-purple' : 'text-brand-gray hover:text-brand-navy'
                        }`}
                      >
                  ...
                  {activeTab === 'notes' && (
                    <div>
                      <textarea
                        ...
                        className="w-full h-64 p-4 border border-brand-border rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-brand-purple focus:border-transparent bg-bg-card text-brand-navy"
                      />
                      <p className="text-xs text-brand-gray/60 mt-2">Notes are saved locally and synced across your devices.</p>
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      {lesson.contentType === 'video' && (
                        <div className="flex items-center gap-4 p-4 bg-bg-secondary rounded-xl">
                          ...
                          <div>
                            <p className="font-medium text-brand-navy">Video Lesson</p>
                            <p className="text-sm text-brand-gray">Duration: {formatDuration(lesson.duration * 60)}</p>
                          </div>
                        </div>
                      )}
                    </div>
