...
//
// Wishlist page — fetches live wishlist data from the API and renders it.
...
import { wishlistService, type WishlistItemUI } from '../services/wishlistService';
...
export const Wishlist: React.FC<WishlistProps> = ({
  ...
  const [wishlistItems, setWishlistItems] = useState<WishlistItemUI[]>([]);
  ...
  const handleRemove = (courseId: string) => {
    ...
    setWishlistItems((prev) => prev.filter((i) => i.courseId !== courseId));
  };
        ...
        {loading ? (
          ...
          >
            {wishlistItems.map((item) => {
              const isAddedToCart = cart.includes(item.courseId);
              return (
                <motion.div
                  key={item.courseId}
                  layout
